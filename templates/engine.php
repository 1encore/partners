...
if(isset($_GET['act'])){
 if($_GET['act'] == 'login'){
    $email = $_POST['email'];
    $pwd = sha1($_POST['pwd']);
    $query_login = $connection->query("SELECT * FROM companies WHERE
    ...
    $query_duplicate = $connection->query("SELECT * FROM companies WHERE email = \"$email\" AND active = 1");
    if($row_duplicate = $query_duplicate->fetch_object()){
      header('Location:index.php?page=registration&error=2');
    }else{
      if($connection->query("INSERT INTO companies VALUES(
        NULL, # id
        \"$name\",
        \"$email\",
        \"$pwd\",
  ...
  }else if($_GET['act'] == 'editProfile' && isset($_SESSION['id'])){
    $com_id = $_SESSION['id'];
    ...
    if(isset($_POST['pwd']) && $_POST['pwd'] != ""){
      ...
      $connection->query("UPDATE companies SET
        password = \"$pwd\",
        name = \"$name\",
        address = \"$addr\",
        phone = \"$phone\",
